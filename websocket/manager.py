...
class ConnectionManager:
  ...
  async def handle_client(self, websocket: WebSocket, client_id: str):
    ...
    try:
      while True:
        ...
        if "text" in message:
          ...
          if msg_type == "heartbeat":
            ...
            # Log connection heartbeat silently to meeting logs
        ...
        # Check binary message
          ...
          if not meeting_id or msg_type not in ("audio", "video"):
            print(f"[WebSocket] Invalid binary header content from client {client_id}")
            continue
          ...
          # Optional: send a brief acknowledgement back to client
          # (Not requested by prompt, but useful for debugging in server.log)
          await store.log_server_message(f"Saved {msg_type} chunk: {filename} ({len(payload_bytes)} bytes)", "INFO")
      ...
      if active_meeting_id:
        store = self.get_or_create_store(active_meeting_id)
        await store.log_server_message(f"Error handling WebSocket client: {str(e)}", "ERROR")
